...
class Asesor extends CI_Controller {
    ...
    public function clientes_proyecto() {
        if ($this->session->userdata('logged_in')) {
            $sesiondata = $this->session->userdata('logged_in');
            $data = array(
                "main" => 'asesor/lista_clientes_proyecto',
...
EOD;
    ...
    public function separacion_inmueble() {
        if ($this->session->userdata('logged_in')) {
            $sesiondata = $this->session->userdata('logged_in');
            $data = array(
                "main" => 'asesor/consultar_ficha_tecnica',
...
//            echo "por aca despues de insertar cuotas restantes";
    ...
    public function buscar_ficha_tecnica() {
        if ($this->session->userdata('logged_in')) {
            $sesiondata = $this->session->userdata('logged_in');
            $data = array(
                "main" => 'asesor/buscar_ficha_tecnica',
    ...
    public function clientes_separacion() {
        if ($this->session->userdata('logged_in')) {
            $sesiondata = $this->session->userdata('logged_in');
            $data = array(
                "main" => 'asesor/clientes_separacion_view',
    ...
    public function mostrar_clientes_separacion($proyecto) {
        if ($this->session->userdata('logged_in')) {
            ...
            $this->load->view('asesor/mostrar_cliente_separacion_view', $data);
        } else {
            redirect('sesion', 'refresh');
        }
    }
    ...
    public function clientes_compra_venta() {
        if ($this->session->userdata('logged_in')) {
            $sesiondata = $this->session->userdata('logged_in');
            $data = array(
                "main" => 'asesor/cliente_compromiso_view',
    ...
    public function mostrar_clientes_compromiso($proyecto) {
        if ($this->session->userdata('logged_in')) {
            ...
            $this->load->view('asesor/mostrar_cliente_compromiso_view', $data);
        } else {
            redirect('sesion', 'refresh');
        }
    }
    ...
    public function clientes_escritura_vivienda() {
        if ($this->session->userdata('logged_in')) {
            $sesiondata = $this->session->userdata('logged_in');
            $data = array(
                "main" => 'asesor/cliente_escritura_view',
    ...
    public function mostrar_clientes_escritura($proyecto) {
        if ($this->session->userdata('logged_in')) {
            ...
            $this->load->view('asesor/mostrar_clientes_escritura_view', $data);
        } else {
            redirect('sesion', 'refresh');
        }
    }
    ...
    public function modelo_separacion_bambu() {
        ...
        $cliente = $this->input->get('nombre') . " " . $this->input->get('apellido');
        $cedula = $this->input->get('cedula');
        ...
        $this->generar_separacion_pdf($cliente, $cedula, "Bambu", $textopdf);
    }
...
EOD;
        ...
        $pdf->writeHTML($texto, true, false, true, false, '');
        $guardarficha = "separaciones/" . $proyecto . "/separacion_de_" . $cliente . "_identificacion_" . $cedula . ".pdf";
        $pdf->Output($guardarficha, 'F');
    }
    ...
    public function compromiso_compraventa() {
        if ($this->session->userdata('logged_in')) {
            $sesiondata = $this->session->userdata('logged_in');
            $data = array(
                "main" => 'asesor/compromiso_compraventa_view',
    ...
    public function mostrar_ficha_tecnica() {
        if ($this->session->userdata('logged_in')) {
            ...
            $sesiondata = $this->session->userdata('logged_in');
            $data = array(
                "main" => 'asesor/mostrar_ficha_tecnica_view',
    ...
    public function get_user_compromiso() {
        if ($this->session->userdata('logged_in')) {
            $cedula = $this->input->get('cedula');
            $proyecto = $this->input->get('proyecto');
            $user = $this->cliente_model->get_compromiso($cedula, $proyecto);
            $datajson = array();
            foreach ($user as $key) {
                $datajson[0] = array($key['cli_nombre'], $key['cli_apellido'], $key['apto_numero'], $key['apto_precio']);
            }
    ...
    public function compromiso_aceptado() {
        if ($this->session->userdata('logged_in')) {
            ...
            $proyecto = $this->input->get('proyecto');
            $apto_num = $this->input->get('apartamento');
            $precioapto = $this->input->get('precioapto');
            $entregaapto = $this->input->get('entregaapto');
            $numsotano = $this->input->get('numsotano');
            $numdeposito = $this->input->get('numdeposito');
            $fechaescritura = $this->input->get('fechaescritura');
            $horaescritura = $this->input->get('horaescritura');
            $notaria = $this->input->get('notaria');
            $nombre_proyecto = $this->cliente_model->nombre_proyecto($proyecto);
            $linderosparticulares = $this->input->get('linderosparticulares');
            $info_cliente = $this->cliente_model->get_usuario_especifico($cedula);
            $info_apto = $this->cliente_model->get_info_apto($proyecto, $apto_num, $cedula);
            $venta_info = $this->cliente_model->get_venta_ficha($cedula, $proyecto);
            $coutas_restantes = $this->cliente_model->get_info_cuotas_ficha($cedula, $venta_info[0]['id_apartamento'], $venta_info[0]['id_entrega']);
            $coutas_iniciales = $this->cliente_model->get_info_coutas_ini_ficha($cedula, $venta_info[0]['id_entrega']);
            $this->load->library("Compromiso_bambu");
            $bambu_compromiso = new Compromiso_bambu();
            $pdf = $bambu_compromiso->crear_pdf_compromiso($nombre, $apellidos, $cedula, $info_cliente[0]['cli_cedula_expedicion'], $info_cliente[0]['est_civil_nombre'], $apto_num, $info_apto[0]["apto_medicion"], $info_apto[0]["apto_precio"], $numdeposito, $info_apto[0]['apto_torre'], $coutas_iniciales, $coutas_restantes, $info_apto[0]['apto_parqueadero'], $numsotano, $fechaescritura, $entregaapto, $linderosparticulares, $horaescritura, $notaria);
            $insertar_apto = array(                
                "apto_estado_venta" => 2,
                "apto_num_deposito" =>$numdeposito,
                "apto_num_sotano" =>$numsotano,
                "apto_fecha_escritura" => $fechaescritura,
                "apto_hora_escritura" => $horaescritura,
                "apto_notaria" => $notaria,
            );
            

            $this->cliente_model->update_apto_estado($insertar_apto, $proyecto, $apto_num,$cedula);
            $insertar_venta = array(               
                "estado_venta_apto" =>2
            );
            $this->cliente_model->update_venta_compromiso($insertar_venta,$cedula,$proyecto);
            $this->pdf_compromiso($pdf, $nombre_proyecto[0]["pro_nombre"], $nombre . "_" . $apellidos, $cedula);
            $this->load->library("Docx");
            $word = new Docx();
            $fileword = "compromisos/" . $nombre_proyecto[0]["pro_nombre"] . "/compraventa_de_" . $nombre . " " . $apellidos . "_" . $cedula;
            $word->generar_docx($pdf, $fileword);
        } else {
            redirect('sesion', 'refresh');
        }
    }

    private function pdf_compromiso($contenido, $proyecto, $cliente, $cedula) {
        $this->load->library('Pdf');

        $pdf = new Pdf(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
        $pdf->SetCreator(PDF_CREATOR);
        $pdf->SetAuthor("Constructora JYP");
        $pdf->SetTitle("Compromiso  " . $cliente . " Proyecto " . $proyecto);
        $pdf->SetSubject("Compromiso " . $cliente . " Proyecto " . $proyecto);
        $pdf->SetKeywords("Compromiso", date("%Y-%mm-%dd"), $cliente, $proyecto);
        $pdf->SetDisplayMode('real', 'default');
        $pdf->Addpage();
        $pdf->SetAutoPageBreak(TRUE, PDF_MARGIN_BOTTOM);
        $texto = <<<EOD
...
EOD;
        ...
        if (!file_exists("compromisos/" . $proyecto)) {
            mkdir("compromisos/" . $proyecto);
        }


        $pdf->writeHTML($texto, true, false, true, false, '');
        $guardarficha = "compromisos/" . $proyecto . "/compraventa_de_" . $cliente . "_identificacion_" . $cedula . ".pdf";
        $pdf->Output($guardarficha, 'F');
        echo "se genero";
    }
    ...
    public function mostrar_separacion_apto() {
        if ($this->session->userdata('logged_in')) {
            $sesiondata = $this->session->userdata('logged_in');
            $data = array(
                "main" => 'asesor/mostrar_separacion_apto_view',
    ...
    public function mostrar_compromiso_apto() {
        if ($this->session->userdata('logged_in')) {
            $sesiondata = $this->session->userdata('logged_in');
            $data = array(
                "main" => 'asesor/mostrar_compromiso_apto_view',
                "titulo" => 'Clientes Con Escrituración De Vivienda',
                "persona" => $this->asesor_model->get_user($sesiondata['username']),
                "clientes" => $this->cliente_model->get_clientes(),
                "proyectos" => $this->cliente_model->get_proyectos_activos(),
    ...
    public function actualizar_informacion() {
        if ($this->session->userdata('logged_in')) {
            $sesiondata = $this->session->userdata('logged_in');
            $data = array(
                "main" => 'asesor/actualizar_informacion_view',
...
//                echo "se realizo satisfactoriamente";
    ...
    public function nuevo_clientes_excel() {
        if ($this->session->userdata('logged_in')) {
            ...
            $sesiondata = $this->session->userdata('logged_in');
            $data = array(
                "main" => 'asesor/clientes_excel_view',
